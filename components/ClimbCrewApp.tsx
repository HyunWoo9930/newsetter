...
export default function ClimbCrewApp() {
  ...
  const [confirmDelVisit, setConfirmDelVisit] = useState<string | null>(null);
  ...
  // 앱 진입 — 딥링크가 있으면 해당 화면으로, 아니면 홈/시작 화면으로
  const LINKABLE = ["home", "explore", "calendar", "profile", "vote", "gymDetail", "crewManage", "probList", "probDetail"];
  const enterApp = (cs: Any[]) => {
    const link = pendingLink.current;
    pendingLink.current = null;
    if (invitePending || !cs.length) { navDepth.current = 0; replaceNav("start"); setScreen("start"); return; }
    let sc = "home";
    const sel: NavSel = {};
    if (link && LINKABLE.includes(link.s)) {
  ...
  // 카카오 세션이 확인되면 로그인 화면을 건너뜀 (딥링크가 있으면 그 화면으로 복귀)
  useEffect(() => {
  ...
  const createCrew = async () => {
    ...
    try {
      ...
      setCrews(cs); setActiveCrewId(c.id); setCrewHomeGymIds([]);
  ...
  // 방문 기록 — 실수 탭 방지를 위해 날짜 확인 시트를 거침
  ...
  const deleteVisit = async (id: string) => {
    ...
    try {
      await api.del(`/api/crews/${activeCrewId}/visits/${id}`);
      setConfirmDelVisit(null);
  ...
  const leaveCrew = async () => {
    ...
    try {
      ...
      else { setActiveCrewId(null); setCrewDetail(null); setCrewGyms([]); setPolls([]); setVisits([]); setRequests([]); tab("start"); }
    } catch (e: Any) { showToast(e.message); }
  };
  ...
  // 다음 세션 = 오늘 이후로 확정된 투표만 (지난 세션이 "다음 세션"으로 계속 남지 않게)
  const todayZero = new Date(); todayZero.setHours(0, 0, 0, 0);
  const confirmedPoll = polls.find((p) => p.confirmedDate && new Date(p.confirmedDate).getTime() >= todayZero.getTime());
  const myRole = crewDetail?.members?.find((m: Any) => m.userId === me?.id)?.role ?? null;
  // 마감·삭제는 되돌릴 수 없으므로 크루장 또는 투표 생성자만 (서버에서도 검증)
  const canClose = !!(me && openPoll && (openPoll.creatorId === me.id || myRole === "LEADER"));
  ...
  const visitRow = (v: Any, future: boolean) => (
    <div key={v.id} onClick={() => openGym(v.gym.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: 14, ...cardStyle, cursor: "pointer" }}>
      ...
      {confirmDelVisit === v.id ? (
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }} onClick={(e) => e.stopPropagation()}>
          <button onClick={() => deleteVisit(v.id)} style={{ height: 38, padding: "0 12px", fontSize: 13, fontWeight: 700, ...crayonBtn(CRAYON.red, 1) }}>삭제</button>
          <button onClick={() => setConfirmDelVisit(null)} style={{ height: 38, padding: "0 10px", border: `2px solid ${INK}`, borderRadius: WOBS[2], background: "#FFFEFA", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>취소</button>
        </div>
      ) : (
        <button aria-label="방문 기록 삭제" onClick={(e) => { e.stopPropagation(); setConfirmDelVisit(v.id); }} style={{ width: 40, height: 40, flexShrink: 0, border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2M7 7l1 13h8l1-13" stroke="#A8A297" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </button>
      )}
    </div>
  );
  ...
  // 마감 프리뷰 — 서버와 같은 규칙(최다 득표 · 동점이면 이른 날짜)으로 뭘로 확정되는지 미리 보여줌
  const voteCnt = (o: Any) => o._count?.votes ?? o.votes?.length ?? 0;
  const closeWinDate = (() => {
    const ds = votePoll?.dateOptions ?? [];
    if (!ds.length) return null;
    return [...ds].sort((a: Any, b: Any) => voteCnt(b) - voteCnt(a) || new Date(a.date).getTime() - new Date(b.date).getTime())[0];
  })();
        ...
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", position: "relative" }}>
          ...
          {is("start") && (
            <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", padding: "96px 24px 32px", animation: "ccfade .3s ease" }}>
              ...
              <div style={{ fontSize: 15, color: "#78756B", marginTop: 8, lineHeight: 1.5 }}>{crews.length ? <>새 크루를 만들거나<br />초대 코드로 다른 크루에 참여할 수 있어요.</> : <>아직 소속된 크루가 없어요.<br />새로 만들거나 초대 코드로 참여해보세요.</>}</div>
              <div style={{ flex: 1 }} />
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {openPolls.map((p) => { const exp = deadlinePassed(p); const responded = p.responderCount ?? 0; return (
                      <div key={p.id} style={{ padding: 18, ...cardStyle }}>
                        ...
                        <div style={{ fontSize: 13, color: "#78756B", marginTop: 8 }}>{responded}/{memberCount}명 응답 완료{exp ? " · 마감하고 확정해주세요" : ""}</div>
              ...
              <div style={{ padding: "22px 16px 0" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {toGo.length === 0 && (!crewLoaded ? (
                    ...
                    <div style={{ padding: 14, ...cardStyle, fontSize: 13, color: "#78756B" }}>홈 암장을 설정하면 갈 때가 된 곳을 알려드려요. <span onClick={openCrewManage} style={{ color: "#E24D3A", fontWeight: 700, cursor: "pointer" }}>설정하기</span></div>
                  ) : (
                    <div style={{ padding: 14, ...cardStyle, color: "#78756B", fontSize: 13 }}>다 다녀왔어요! 🎉</div>
                  ))}
                  {toGo.map((gg) => (
            ...
            <div style={{ animation: "ccfade .3s ease", height: "100%", display: "flex", flexDirection: "column" }}>
              ...
              {exploreView === "list" ? (
                <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "2px 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
                  {exploreGyms.length === 0 && <div style={{ padding: 14, ...cardStyle, color: "#78756B", fontSize: 13 }}>{crewLoaded ? "조건에 맞는 암장이 없어요." : "불러오는 중이에요…"}</div>}
                  {exploreGyms.map((g) => (
                    <div key={g.id} onClick={() => openGym(g.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: 14, ...cardStyle, cursor: "pointer", flexShrink: 0 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#78756B", marginTop: 2 }}>{g.loc}{g.rating ? ` · ★ ${g.rating}` : ""}</div>
                        <div style={{ display: "inline-flex", alignItems: "center", marginTop: 6, padding: "2px 8px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: g.due ? "#FBEFD9" : "#F3EEDF", color: g.due ? "#B5730A" : "#78756B" }}>{visitLabel(g)}</div>
                      </div>
                      <ChevR />
                    </div>
                  ))}
                ...
                <div style={{ padding: "56px 16px 8px" }}>
                  ...
                  {openPolls.length > 1 && (
                    <div style={{ display: "flex", gap: 8, overflowX: "auto", marginBottom: 12, paddingBottom: 2 }}>
                      {openPolls.map((p) => { const on = p.id === openPoll?.id; return (
                        <button key={p.id} onClick={() => { setSelPollId(p.id); replaceNav("vote", { poll: p.id }); }} style={{ flexShrink: 0, padding: "6px 14px", fontSize: 14, fontWeight: 700, cursor: "pointer", border: on ? `2px solid ${INK}` : "2px dashed rgba(58,54,51,0.3)", borderRadius: WOBS[2], background: on ? HILITE : "#FFFEFA", color: on ? INK : "#78756B" }}>{p.title}</button>
                      ); })}
                    </div>
                  )}
                  ...
                  {openVote?.expired && (
                    <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: WOBS[2], border: `2px solid ${INK}`, background: "#FBF0DA", fontSize: 14, fontWeight: 700, color: "#B4432E" }}>
                      투표 기한이 지났어요. {canClose ? "아래에서 마감하고 확정해주세요." : "크루장이나 투표를 만든 사람이 마감할 수 있어요."}
                    </div>
                  )}
                  {openVote && (
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 14 }}>
                      ...
                      {canClose && <button onClick={() => setDeletePollSheetOpen(true)} style={{ height: 42, padding: "0 12px", border: "none", background: "transparent", fontSize: 14, fontWeight: 700, color: "#B4432E", cursor: "pointer", textDecoration: "underline" }}>삭제</button>}
                    </div>
                  )}
                </div>
                ...
                {openVote && (<>
                  ...
                  {(!hasGymOpts || voteTab === "date") && (
                    <div style={{ padding: "18px 16px 0" }}>
                      <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 2 }}>{voteSubmitted ? "제출 완료 · 날짜를 누르면 누가 되는지 보여요" : "되는 날에 동그라미 쳐주세요"}</div>
                      <div style={{ fontSize: 14, color: "#78756B", marginBottom: 16 }}>{voteSubmitted ? "선택을 바꾸려면 아래 '수정하기'를 눌러주세요" : "여러 날도 좋아요 · 누르면 아래에 누가 되는지 나와요"}</div>
                    ...
                    <div style={{ padding: "18px 16px 0" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {gymOpts.map((o: Any) => { const sel = !!voteGyms[o.id]; return (
                          <div key={o.id} onClick={() => { if (voteSubmitted) { showToast("아래 '수정하기'를 누르면 바꿀 수 있어요"); return; } setVoteGyms((v) => ({ ...v, [o.id]: !v[o.id] })); }} style={{ ...rowStyle(sel), alignItems: "flex-start" }}><div style={{ ...boxStyle(sel), marginTop: 1 }}>{sel ? "✓" : ""}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 600 }}>{o.name}</div><div style={{ fontSize: 12, color: "#78756B", marginTop: 2 }}>{o.meta}</div>{voterRow(o.voters)}</div><div style={{ fontSize: 13, fontWeight: 700, color: "#78756B" }}>{o.count}표</div></div>
                        ); })}
                      </div>
                    </div>
                  )}
          ...
          {is("calendar") && (
            <div style={{ animation: "ccfade .3s ease", paddingBottom: 96 }}>
              ...
              {visits.length === 0 && <div style={{ padding: "22px 16px 0" }}><div style={{ padding: 14, ...cardStyle, color: !crewLoaded ? "#A8A297" : "#78756B", fontSize: 13 }}>{!crewLoaded ? "불러오는 중이에요…" : "방문 기록이 아직 없어요."}</div></div>}
          ...
          {is("profile") && (
            <div style={{ animation: "ccfade .3s ease", paddingBottom: 96 }}>
              ...
              <div style={{ padding: "18px 16px 0" }}>
                <div style={{ display: "flex", ...cardStyle, overflow: "hidden" }}>{[[String(visits.length), "크루 방문"], [String(gyms.filter((g) => g.ever).length), "크루 방문 암장"], [String(me?.stats?.myReviewCount ?? 0), "내 리뷰"]].map(([n, l], i) => (<div key={l} style={{ flex: 1, textAlign: "center", padding: "16px 0", borderLeft: i ? "2px dashed rgba(58,54,51,0.25)" : "none" }}><div style={{ fontSize: 20, fontWeight: 800 }}>{n}</div><div style={{ fontSize: 12, color: "#78756B", marginTop: 2 }}>{l}</div></div>))}</div>
                <div style={{ fontSize: 11, color: "#A8A297", marginTop: 6, textAlign: "center" }}>방문 수치는 {ac?.name ?? "현재 크루"} 기준이에요</div>
              </div>
          ...
          {is("crewManage") && (
            <div style={{ animation: "ccfade .3s ease", paddingBottom: 100 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "56px 12px 4px" }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>크루 관리</div><div style={{ fontSize: 13, color: "#78756B" }}>{ac?.name ?? ""}</div></div>
        ...
        {closeSheetOpen && (
          <>
            ...
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 130, background: "#FFFEFA", borderTop: `2.5px solid ${INK}`, borderRadius: "28px 22px 0 0 / 24px 28px 0 0", padding: "10px 16px 30px", boxShadow: "0 -8px 30px rgba(58,54,51,0.16)", animation: "ccsheet .28s cubic-bezier(.2,.8,.2,1)" }}>
              ...
              <div style={{ fontSize: 13, color: "#78756B", margin: "0 4px 12px", lineHeight: 1.5 }}>마감하면 아래 내용으로 확정되고 크루 방문 일정에 등록돼요. 되돌릴 수 없어요.</div>
              <div style={{ margin: "0 0 14px", padding: "14px 16px", ...cardStyle, borderRadius: WOBS[2] }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#78756B" }}>이렇게 확정돼요</div>
                <div style={{ fontSize: 20, fontWeight: 800, marginTop: 4 }}>{closeWinDate ? fmtDate(closeWinDate.date) : "날짜 후보 없음"}{closeWinDate ? <span style={{ fontSize: 13, fontWeight: 700, color: "#78756B" }}> · {voteCnt(closeWinDate)}표</span> : null}</div>
                <div style={{ fontSize: 15, fontWeight: 700, marginTop: 4, color: closeWinGym ? INK : "#78756B" }}>{closeWinGym ? <>{closeWinGym.name} <span style={{ fontSize: 13, color: "#78756B" }}>· {closeWinGym.count}표</span></> : "암장 후보 없음 · 날짜만 확정돼요"}</div>
                {respondedCount === 0 && <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: "#B4432E" }}>⚠ 아직 아무도 응답하지 않았어요 — 이대로 확정하면 가장 이른 날짜로 정해져요</div>}
              </div>
        ...
        {deletePollSheetOpen && (
          <>
            ...
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 130, background: "#FFFEFA", borderTop: `2.5px solid ${INK}`, borderRadius: "28px 22px 0 0 / 24px 28px 0 0", padding: "10px 16px 30px", boxShadow: "0 -8px 30px rgba(58,54,51,0.16)", animation: "ccsheet .28s cubic-bezier(.2,.8,.2,1)" }}>
              ...
              <div style={{ fontSize: 13, color: "#78756B", margin: "0 4px 16px", lineHeight: 1.5 }}>&apos;{openVote?.title ?? ""}&apos; 투표와 지금까지의 응답이 모두 지워져요. 되돌릴 수 없어요.</div>
        ...
        {visitSheetOpen && (
          <>
            ...
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 130, background: "#FFFEFA", borderTop: `2.5px solid ${INK}`, borderRadius: "28px 22px 0 0 / 24px 28px 0 0", padding: "10px 16px 30px", boxShadow: "0 -8px 30px rgba(58,54,51,0.16)", animation: "ccsheet .28s cubic-bezier(.2,.8,.2,1)" }}>
              ...
              <div style={{ fontSize: 13, color: "#78756B", margin: "0 4px 14px" }}>{sg?.name ?? ""} · 크루 전체 기록에 추가돼요</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#78756B", margin: "0 4px 8px" }}>언제 갔어요?</div>
            ...
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 130, background: "#FFFEFA", borderTop: `2.5px solid ${INK}`, borderRadius: "28px 22px 0 0 / 24px 28px 0 0", padding: "10px 16px 30px", boxShadow: "0 -8px 30px rgba(58,54,51,0.16)", animation: "ccsheet .28s cubic-bezier(.2,.8,.2,1)", maxHeight: "85%", overflowY: "auto" }}>
              ...
              {([["크루 이름", "name"], ["한 줄 소개", "bio"], ["주 활동 지역", "region"], ["오픈카톡 링크", "kakao"]] as const).map(([label, key]) => (
                <div key={key} style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#78756B", marginBottom: 6 }}>{label}</div>
        ...
        {leaveSheetOpen && (
          <>
            ...
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 130, background: "#FFFEFA", borderTop: `2.5px solid ${INK}`, borderRadius: "28px 22px 0 0 / 24px 28px 0 0", padding: "10px 16px 30px", boxShadow: "0 -8px 30px rgba(58,54,51,0.16)", animation: "ccsheet .28s cubic-bezier(.2,.8,.2,1)" }}>
              ...
              <div style={{ fontSize: 13, color: "#78756B", margin: "0 4px 16px", lineHeight: 1.5 }}>
                {myRole === "LEADER" && members.length <= 1
                  ? `'${ac?.name ?? ""}' 크루와 투표·방문 기록이 모두 지워져요. 되돌릴 수 없어요.`
                  : myRole === "LEADER"
